...
public class TargetESP extends ModuleStructure implements IMinecraft {
    ...
    // Хранилище атакованных игроков с временем последнего удара
    private final Map<LivingEntity, Long> attackedEntities = new ConcurrentHashMap<>();
    private static final long DECAY_TIME = 10000; // 10 секунд до исчезновения
    ...
    // Обработчик события атаки - добавляет игрока в список отслеживаемых
    @EventHandler
    public void onAttack(AttackEvent event) {
        if (event.getTarget() instanceof LivingEntity target && target.isAlive()) {
            ...
            // Если это первый атакованный игрок, устанавливаем его как цель
            if (lastTarget == null) {
                lastTarget = target;
                smoothedPos = target.getLerpedPos(1.0f);
            }
        }
    }
    ...
    public void onRender3D(WorldRenderEvent e) {
        ...
        // Очищаем умерших игроков и тех, у кого истекло время
        long currentTime = System.currentTimeMillis();
        attackedEntities.entrySet().removeIf(entry -> {
            LivingEntity entity = entry.getKey();
            long lastAttackTime = entry.getValue();
            return !entity.isAlive() || (currentTime - lastAttackTime) > DECAY_TIME;
        });

        // Выбираем цель для отрисовки (последнего атакованного игрока)
        LivingEntity target = null;
        if (!attackedEntities.isEmpty()) {
            // Находим игрока с самым свежим временем атаки
            long latestTime = 0;
            for (Map.Entry<LivingEntity, Long> entry : attackedEntities.entrySet()) {
